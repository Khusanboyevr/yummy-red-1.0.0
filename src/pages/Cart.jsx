...
import { Trash2, ArrowLeft, CheckCircle, MapPin, User, Phone, CreditCard, Upload, Loader2, Send } from 'lucide-react';

// TELEGRAM BOT SOZLAMALARI (BotFather'dan olingan ma'lumotlarni kiriting)
const BOT_TOKEN = "7857475586:AAEA1yRlY1QXtaqnbD6eHUGCgPhBjf0naBI"; // Real Bot Tokeningiz
const CHAT_ID = "6112428725"; // Real Chat ID'ingiz (Mijozning yangi ID-si)
...
export default function Cart() {
  ...
  const sendOrderToTelegram = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.description || 'Telegramga yuborishda noma\'lum xatolik');
      }
  ...
  if (checkoutStep === 'success') {
    return (
      <div className="container text-center py-5 mt-5">
        ...
        <p className="text-muted mb-4">Tez orada operatorimiz siz bilan bog'lanadi. Rahmat!</p>
        <Link to="/" className="btn btn-danger px-5 py-2" style={{ borderRadius: '25px' }}>Asosiy Sahifa</Link>
      </div>
    );
  }

  return (
    <section className="section" style={{ minHeight: '60vh', marginTop: '100px' }}>
      <div className="container" data-aos="fade-up">
        <div className="section-title">
          <h2>
            {checkoutStep === 'info' ? 'Ma\'lumotlarni Kiriting' : 
             checkoutStep === 'payment' ? 'To\'lov va Tasdiqlash' : 'Xaridlar'}
          </h2>
          <p>
            <span>{checkoutStep ? 'Buyurtma' : 'Sizning'}</span> 
            <span className="description-title">{checkoutStep ? 'Berish' : 'Savatingiz'}</span>
          </p>
        </div>
        ...
        {cart.length === 0 && !checkoutStep ? (
          <div className="text-center py-5">
            ...
            <Link to="/menu" className="btn btn-danger" style={{ borderRadius: '20px', padding: '10px 30px' }}>
          ...
          <div className="row gy-4">
              ...
              {!checkoutStep ? (
                <div className="card shadow-sm border-0">
                  <div className="card-body p-4">
                    {cart.map((item, index) => (
                      <div key={index} className="d-flex align-items-center mb-4 pb-4 border-bottom">
                        <img src={item.image} alt={item.name} className="img-fluid rounded" style={{ width: '80px', height: '80px', objectFit: 'cover' }} />
                        <div className="ms-3 flex-grow-1">
                          <h5 className="mb-1">{item.name}</h5>
                          {item.size && <span className="badge bg-secondary mb-2">{item.size}</span>}
                        </div>
                        <div className="d-flex align-items-center mx-3">
                          <button className="btn btn-sm btn-outline-secondary px-2 py-0" onClick={() => updateQuantity(item._id, item.size, Math.max(1, (item.quantity || 1) - 1))}>-</button>
                          <span className="mx-2">{item.quantity || 1}</span>
                          <button className="btn btn-sm btn-outline-secondary px-2 py-0" onClick={() => updateQuantity(item._id, item.size, (item.quantity || 1) + 1)}>+</button>
                        </div>
                        <div className="text-end ms-3" style={{ minWidth: '100px' }}>
                          <h5 className="text-danger mb-2">{(item.price * (item.quantity || 1)).toLocaleString()} so'm</h5>
                          <button 
                    ...
                    <div className="mt-4">
                      <Link to="/menu" className="text-danger text-decoration-none d-flex align-items-center">
                        <ArrowLeft size={16} className="me-1" /> Yana xarid qilish
                      </Link>
                    </div>
                  </div>
                </div>
                ...
                <div className="card shadow-sm border-0">
                  <div className="card-body p-4">
                    <form onSubmit={handleInfoSubmit}>
                      <div className="mb-3">
                        <label className="form-label d-flex align-items-center gap-2"><User size={18} /> To'liq ismingiz</label>
                        <input 
                          type="text" 
                      ...
                      <div className="mb-3">
                        <label className="form-label d-flex align-items-center gap-2"><Phone size={18} /> Telefon raqamingiz</label>
                        <input 
                          type="tel" 
                          className="form-control" 
                          placeholder="+998" 
                      ...
                      <div className="d-flex gap-3">
                        ...
                        <button type="submit" className="btn btn-danger w-100">Keyingisi</button>
                      </div>
                    </form>
                  </div>
                </div>
            ...
            <div className="col-lg-4">
              <div className="card shadow-sm border-0 bg-light position-sticky" style={{ top: '120px' }}>
                <div className="card-body p-4">
                  ...
                  <div className="d-flex justify-content-between mb-4">
                    ...
                    <strong className="h5 mb-0 text-danger">{calculateTotal().toLocaleString()} so'm</strong>
                  </div>
                  
                  {!checkoutStep && (
                    <button className="btn btn-danger w-100 py-3" onClick={() => setCheckoutStep('info')} style={{ borderRadius: '25px', fontWeight: '600' }}>
